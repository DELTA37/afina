...
std::pair<void*, void*> Simple::findEnoughFree(void* base, size_t N) {
  ...
  while (cur->size < N) {
    ...
    prev = ptr;
    ptr = add_diff(this->base, cur->diff);
    cur = info(ptr);
  }
...
void Simple::insertBlock(void* prev, void* base, size_t size) {
  ...
  FreeSpace* start_info = info(add_diff(this->base, this->size - m));
  void* new_base = NULL;
...
FreeSpace* Simple::registerPointer(ptrdiff_t diff, size_t size) {
  size_t m = sizeof(FreeSpace);
  for (size_t i = this->pointer_info_start; i < this->size - m; i+=m) {
    FreeSpace* ptr_info = info(add_diff(this->base, i));
    if (ptr_info->size == 0) {
      this->setBlock(add_diff(this->base, i), size, diff);
      return info(add_diff(this->base, i));
    } 
  } 
  FreeSpace* last_info = info(this->last_chain);
  if (last_info->size <= 2 * m) {
  ...
  last_info->size -= m;
  this->setBlock(add_diff(this->base, this->pointer_info_start), size, diff);
  return info(add_diff(this->base, this->pointer_info_start));
}

void Simple::unregisterPointer(FreeSpace* memreg) {
  size_t m = sizeof(FreeSpace);
  if (memreg == info(add_diff(this->base, this->pointer_info_start))) {
    this->pointer_info_start += m;
    FreeSpace* last_info = info(this->last_chain);
    last_info->size += m;
    return;
...
void Simple::setFirstBlock(ptrdiff_t _diff) {
  size_t m = sizeof(FreeSpace);
  FreeSpace* inf = info(add_diff(this->base, this->size - m));
  inf->diff = _diff;
  inf->size = 1;
}
...
void Simple::unite(void* cur) {
  ...
  while(cur_info->diff > 0) {
    next = add_diff(this->base, cur_info->diff);
    next_info = info(next);
    if(add_diff(cur, cur_info->size) == next) {
...
void* Simple::findFirstBlock(void) {
  ...
  if (inf->size == 0) {
    ...
    return add_diff(this->base, inf->diff);
  }
}
...
void Simple::realloc(Pointer &pt, size_t N) {
  ...
  if (p.size > N) {
     ...
     */
    void* ptr = add_diff(add_diff(this->base, p.diff), N);
    if (start_info->size == 0) {
      /*
    ...
    if (ptr > base) {
      ...
      prev = add_diff(this->base, this->size - m);
    }
  ...
  void* ptr = add_diff(this->base, p.diff);
  void* base = this->findFirstBlock();
  void* prev = NULL;
  if (base < ptr) {
    prev = this->findPrevious(base, ptr);
  ...
  if (prev_info->diff > 0) {
    next = add_diff(this->base, prev_info->diff);
    this->unite(next);
    next_info = info(next);
    if (
        add_diff(ptr, p.size) == next && N <= p.size + next_info->size && 
      ...
      if (N < p.size + next_info->size) {
        ...
        p.size = N;
        if (prev_info->diff == 0 && prev != add_diff(this->base, this->size - m)) {
          this->last_chain = prev;
        }
        return;
      }
   ...
   */
  Pointer q = alloc(N);
  char* dst = reinterpret_cast<char*>(add_diff(this->base, q.ptr->diff));
  char* src = reinterpret_cast<char*>(add_diff(this->base, p.diff));
...
void Simple::free(Pointer &pt) {
  ...
  if (start_info->size == 0) {
    ...
    start_info->size = 1;
    this->setBlock(add_diff(this->base, p.diff), p.size, 0);
    this->last_chain = add_diff(this->base, p.diff);
  }
  else {
    void* base = this->findFirstBlock();
    void* ptr = add_diff(this->base, p.diff);
...
void Simple::placeTo(FreeSpace* p_inf, ptrdiff_t where) {
  copy(add_diff(this->base, p_inf->diff), add_diff(this->base, where), p_inf->size);
  p_inf->diff = where;
}
...
FreeSpace* Simple::getNext(ptrdiff_t l) {
  ...
  for (size_t i = this->pointer_info_start; i < this->size - m; i+=m) {
    FreeSpace* inf = info(add_diff(this->base, i));
...
void Simple::defrag() {
  ...
  base = add_diff(this->base, start_info->diff);
  void* cur = base;
  FreeSpace* cur_info = info(cur);
  while (cur_info->diff > 0) {
    this->unite(cur);
    ...
    cur = add_diff(this->base, cur_info->diff);
  }
  /*
   * moving
   */
  ...
  this->last_chain = add_diff(this->base, len);
